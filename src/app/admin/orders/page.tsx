        ...
        <div className="sticky top-0 z-20 px-6 py-4 flex items-center gap-4 border-b bg-[#080810] border-white/5">
          ...
          <button 
            onClick={async () => {
              const res = await fetch('/api/orders');
              const data = await res.json();
              alert(`تم التوصيل بقاعدة البيانات. عدد الطلبات الحالي: ${Array.isArray(data) ? data.length : 'خطأ في البيانات'}`);
            }}
            className="px-4 py-2 rounded-xl bg-blue-500/20 text-blue-400 text-xs font-bold border border-blue-500/20 hover:bg-blue-500/30 transition"
          >
            اختبار الاتصال 🔌
          </button>
